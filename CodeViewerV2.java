...
public class CodeViewer extends GhidraScript {
    ...
    private class GhidraAppFrame extends JFrame {
        ...
        private void loadDecompiledCode(Function function) {
            ...
            String coutString = "std::operator<<((basic_ostream *)std::cout";
            String constructorString = "std::__cxx11::basic_string<char,std::char_traits<char>,std::allocator<char>>";
            String removeString1 = "std::allocator<char>::~allocator";
            String removeString2 = "std::__cxx11::basic_string<char,std::char_traits<char>,std::allocator<char>>::~basic_string";
            String removeString3 = "std::allocator<char>::allocator()";
            String removeString4 = "endl";
            String removeString5 = "(in_FS_OFFSET + 0x28)";
            String foundString = "";
            boolean searchStringFound = false;
            String[] lines = decompiledCode.toString().split(";"); // Split by newlines

            for (String line : lines) {
            	if (line.contains(removeString1) || line.contains(removeString2) || line.contains(removeString3) || line.contains(removeString5)) {
            		decompiledCode = new StringBuilder(decompiledCode.toString().replaceAll(Pattern.quote(line), Matcher.quoteReplacement("\n")));
            	} else if (line.contains(removeString4)) {
            		popup(line);
                    searchStringFound = true;
                    foundString = line;
                    String newString = "std::endl";
                    decompiledCode = new StringBuilder(decompiledCode.toString().replaceAll(Pattern.quote(line), Matcher.quoteReplacement("\n" + "  " + newString)));
                    popup(newString);
                    ...
                    popup(newString);
                    //break;
                } else if (line.contains(coutString)) {
                	popup(line);
                    searchStringFound = true;
                    foundString = line;
                    String newString = coutFormat(foundString + ";");
                    decompiledCode = new StringBuilder(decompiledCode.toString().replaceAll(Pattern.quote(line), Matcher.quoteReplacement("\n" + "  " + newString)));
                    popup(newString);
                } else if (line.contains(constructorString)) {
                	popup(line);
                    searchStringFound = true;
                    foundString = line;
                    String newString = constructFormat(foundString + ";");
                    decompiledCode = new StringBuilder(decompiledCode.toString().replaceAll(Pattern.quote(line), Matcher.quoteReplacement("\n" + "  " + newString)));
                    popup(newString);
                }
            }
        ...
        private String constructFormat(String originalCode) {
            // Find the index of '(' in the original code
            int openingParenIndex = originalCode.indexOf('(');
            ...
            if (openingParenIndex != -1) {
                // Find the index of ',' and ')' after '('
                int commaIndex = originalCode.indexOf(',', openingParenIndex);
                int semicolonIndex = originalCode.indexOf(',', commaIndex + 1);

                if (commaIndex != -1 && semicolonIndex != -1) {
                    // Extract substrings for first and second parameters
                    String firstParam = originalCode.substring(openingParenIndex + 1, commaIndex).trim();
                    String secondParam = originalCode.substring(commaIndex + 1, semicolonIndex).trim();

                    // Return the formatted string
                    return "String " + firstParam + " = " + secondParam;
                }
            }
        ...
        private String coutFormat(String originalCode) {
            // Find the index of '(' in the original code
            int commaIndex = originalCode.indexOf(',');
            int semicolonIndex = originalCode.indexOf(';', commaIndex);
            if (commaIndex != -1 && semicolonIndex != -1) {
            	String param = originalCode.substring(commaIndex + 1, semicolonIndex - 1).trim();
            	// Return the formatted string
                return " std::cout << " + param;
            }
